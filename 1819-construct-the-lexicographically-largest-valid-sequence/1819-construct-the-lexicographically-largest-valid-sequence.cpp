...
public:
    void clc(int n, vector<int>& v, vector<int>& pre, vector<int>& res,
        ...
        if (check) return;  // Stop when a valid sequence is found
        ...
        for (int num : pre) {
            if (st.count(num)) continue;  // Skip already placed numbers
            ...
            if (num > 1 && idx + num < v.size() && v[idx + num] == -1) {
                v[idx + num] = num;
                clc(n, v, pre, res, st, check, idx + 1);
                if (check) return;  // Stop if sequence found
                v[idx + num] = -1;  // Backtrack
            } else if (num == 1) { // '1' appears only once, no second position needed
                clc(n, v, pre, res, st, check, idx + 1);
                if (check) return;
            }
    ...
    vector<int> constructDistancedSequence(int n) {
        vector<int> pre;
        for (int i = n; i >= 1; i--) pre.push_back(i);
