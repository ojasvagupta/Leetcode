...
public:
    ...
    ll countValidNumbers(int idx, int tight, string& numStr, string& suffix,
                         int limit, int numLength, vector<vector<ll>>& dp) {
        if (idx == numLength)
            return 1;
        if (dp[idx][tight] != -1)
            return dp[idx][tight];
        ...
        for (int i = lowerBound; i <= upperBound; i++)
            count +=
                countValidNumbers(idx + 1, tight && (i == numStr[idx] - '0'),
                                  numStr, suffix, limit, numLength, dp);
    ...
    ll numberOfPowerfulInt(ll start, ll finish, int limit, string s) {
        ...
        if (s.size() <= upperLength)
            upperCount =
                countValidNumbers(0, 1, upperLimit, s, limit, upperLength, dp);

        dp.assign(17, vector<ll>(2, -1));
        if (s.size() <= lowerLength)
            lowerCount =
                countValidNumbers(0, 1, lowerLimit, s, limit, lowerLength, dp);
